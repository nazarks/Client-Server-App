    ...
    def save_data(self):
        ...
        if not self.client_passwd.text():
            ...
            self.messages.information(
                self, "Alert", "Пользователь уже существует. Обновляем пароль"
            )

        # Пользователь и хеш пароля записать в базу данных
        passwd_hash = self.server.get_hash(
            username=self.client_name.text(),
            password=self.client_passwd.text(),
        )
        self.database.add_update_user(
            username=self.client_name.text(), passwd_hash=passwd_hash
        )
