...
class ClientMainWindow(QMainWindow):
    ...
    def history_list_update(self):
        ...
        messages = self.database.get_messages_by_in_out(contact=self.current_chat)
        logger.debug("history_list_update %s for user %s", str(messages), self.transport.username)
        ...
        for message in messages:
            if message[4] == "in":
                mess = QStandardItem(f"Входящее от {datetime_from_utc_to_local(message[3])}:\n {message[2]}")
                mess.setEditable(False)
                mess.setBackground(QBrush(QColor(255, 213, 213)))
                mess.setTextAlignment(Qt.AlignLeft)
                self.history_model.appendRow(mess)
            else:
                mess = QStandardItem(f"Исходящее от {datetime_from_utc_to_local(message[3])}:\n {message[2]}")
                mess.setEditable(False)
    ...
    def select_active_user(self):
        # Выбранный пользователем (даблклик) находится в выделенном элементе в QListView
    ...
    def add_contact_window(self):
        ...
        add_contact_dialog.btn_ok.clicked.connect(lambda: self.add_contact_action(add_contact_dialog))
    ...
    def send_message(self):
        ...
        if not message_text:
            return
        try:
            self.transport.sent_message_to_user(msg=message_text, to_user=self.current_chat)
        except OSError as err:
            if err.errno:
                self.messages.critical(self, "Ошибка", "Потеряно соединение с сервером!")
                self.close()
            else:
                # Освобождаем сокет по Timeout, сервер не вернул ошибок значит все хорошо
                # self.messages.critical(self, "Ошибка", "Таймаут соединения!")
                self.database.save_message(
                    from_user=self.transport.username, to_user=self.current_chat, message=message_text
                )
    ...
    def connection_lost(self):
        self.messages.critical(self, "Сбой соединения", "Потеряно соединение с сервером. ")
        self.close()
